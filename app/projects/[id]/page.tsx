...
export async function generateStaticParams() {
  ...
  const mdxFiles = files.filter((file) => file.endsWith('.mdx') && !file.startsWith('_')).map((file) => path.join(projectsPath, file));
...
export default async function ProjectPage({ params, searchParams }: ProjectPageProps) {
  ...
  return (
    <ProjectClient title={data.title} date={data.date} techStack={data.techStack} images={imageList} mdxSource={mdxSource} showSlider={showSlider} />
  );
}
