---
title: '2022 Qatar World Cup Winner Prediction (Simulation)'
date: '20221101'
techStack: 'Python, Pandas, Scikit-learn, XGBoost'
id: 2022-qatar-world-cup
hidden: false
images:
  - src: '/photo/proj/2022-qatar-world-cup/2022-qatar-worldcup.jpg'
    title: 'Cover'
...
- **Problem:** Estimate match outcomes for the 2022 Qatar World Cup and simulate the tournament bracket using historical results and FIFA ranking data.
- **My role:** I built the data preparation, feature engineering, model comparison, tournament simulation, and result visualization in a Jupyter Notebook.
- **Key decisions:** I compared a simple Logistic Regression baseline with more complex models and treated the output as a probability-based exploration rather than a certain prediction.
- **Outcome:** Logistic Regression performed best on this dataset, and the final simulation selected Brazil as the most likely winner.

## Core Functionality

- Normalize historical match, team, and FIFA ranking datasets
- Create ranking, goal-difference, and win-rate features
- Compare classifiers for match-outcome prediction
- Simulate group and knockout stages from predicted probabilities

## Tech Stack & Why

- **Python + Pandas** — Data cleaning, joins, aggregation, and feature preparation.
- **Scikit-learn** — Reproducible train/test split, model evaluation, and pipeline composition.
- **XGBoost** — A gradient-boosting baseline for comparison with the linear model.
- **Matplotlib + NetworkX** — Performance charts and tournament-bracket visualization.

## Modeling Flow

`Historical matches + FIFA rankings → feature engineering → model evaluation → match probabilities → group/knockout simulation → bracket visualization`

1. Align rankings with match dates and normalize country names across datasets.
2. Build team and match features, including rank difference, average rank, goal difference, and non-friendly status.
3. Compare Logistic Regression, XGBoost, Gradient Boosting, AdaBoost, and SVC models.
4. Use the selected model's probabilities to advance teams through group and knockout rounds.

## Design Decisions

- **Pipeline pattern** — Data preparation, feature generation, modeling, and simulation are separated into sequential stages, making each stage inspectable and replaceable.
- **Baseline-first model selection** — A simple Logistic Regression baseline was evaluated alongside more complex models instead of assuming a complex model would perform best.
- **Probabilistic simulation** — The output is treated as an exploration of likely outcomes, not a deterministic prediction.

## Result

Logistic Regression performed best on this dataset, and the final bracket simulation selected Brazil as the most likely winner. The notebook also highlights that ranking-based features and historical samples limit the certainty of the result.

## Data Sources

- Base project: https://www.kaggle.com/code/agostontorok/soccer-world-cup-2018-winner/notebook
...
- Modified data window to focus on the last 10 years
- Worked in Google Colab and Jupyter Notebook
...
Offense Score = avg(home score) _ 0.3 + avg(away score) _ 0.7
...
![output](/photo/proj/2022-qatar-world-cup/nb-cell-042-out-00.png)
...
![output](/photo/proj/2022-qatar-world-cup/nb-cell-045-out-00.png)
...
![output](/photo/proj/2022-qatar-world-cup/nb-cell-056-out-00.png)

## Conclusion

**The winner of my 2022 Qatar World Cup simulation is Brazil.**

This notebook builds a simple match outcome model from historical results and FIFA rankings, then simulates the tournament bracket to estimate likely winners. The model selection step favored logistic regression for this dataset, and the final visualization summarizes the knockout path and odds.

## Notes

- The simulation is sensitive to ranking features and historical results, so it is better viewed as a probabilistic exploration rather than a definitive forecast.
- Future improvements could include player-level metrics, recent form weighting, and repeated Monte Carlo runs to provide confidence intervals.
