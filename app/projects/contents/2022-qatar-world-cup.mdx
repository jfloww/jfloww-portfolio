...
This project simulates the 2022 Qatar World Cup by modeling match outcomes from historical results and FIFA ranking features, then running a bracket-style simulation to estimate the most likely winner. The goal was to explore a clear, explainable pipeline using a small, focused feature set.
...
Predict match outcomes for the 2022 Qatar World Cup using historical results and ranking-based features, then run a full tournament simulation.

### 2. Approach

Engineer team-level features (goal difference, ranks, win rate), compare multiple classifiers, then simulate the group and knockout stages using the selected model.

### 3. Implementation

- Load and normalize match and ranking datasets
- Build features and compute team metrics
- Train and evaluate multiple models
- Simulate group stage and knockout bracket
- Visualize bracket outcomes

## Results

Logistic regression performed best on this dataset, and the final bracket simulation predicted Brazil as the most likely winner.
